...
public class ItemServiceImpl extends BaseDAO {
    ...
    /**
     * used by item nikasa entry and stock query
     */
    public List<Item> itemStockQuery(String itemName, int categoryId, int vendorId, String panaNum, String rackNumber, String khataNumber, String dakhilaNumber, Date fromDate, Date toDate, List<String> specs) throws Exception {
    ...
    public List<Item> getAddedItems() throws Exception {
        Criteria c = getSession().createCriteria(Item.class);
        // c.createAlias("item", "it");
        c.add(Restrictions.eq("dFlag", 1));
        c.add(Restrictions.ne("accountTransferStatus", Item.ACCOUNT_TRANSFERRED_TO_NEW));
     ...
     * @throws Exception
     */
    public List<Item> getAllItemsToCloseCurrFiscalYear(int type) throws Exception {
        Criteria c = getSession().createCriteria(Item.class);
        c.add(Restrictions.eq("dFlag", 1));
        if (type > 0) {
